...
from app.services.vector_store_manager import embed_and_store_chunks
from app.api.models import IngestionResponse, ChunkingStrategy, ChatResponse, ConversationMode, KnowledgeBaseMode
...
rag_router = APIRouter(
    prefix="/RAG",
    tags=["Conversational RAG API"]
)

@rag_router.post("/chat", response_model=ChatResponse)
async def conversational_rag(
    query: Annotated[str, Form(...)],
    mode: Annotated[ConversationMode, Form()] = ConversationMode.RESTART,
    knowledge_base: Annotated[KnowledgeBaseMode, Form()] = KnowledgeBaseMode.NO,
